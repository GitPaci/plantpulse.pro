...
| **Process Setup** | Stage Types, Stage Defaults, Turnaround Activities, Shutdowns | Configure process parameters: define stage types (Inoculum, Seed n-2, Seed n-1, Production) with full CRUD, edit default stage durations per product line (header shows shortName) with target/min/max and dynamic stage type dropdown, define turnaround activities (CIP/SIP/Cleaning) per equipment group with d:h:m picker (pre-populated defaults for all groups), manage plant shutdown periods with date range, reason, and conflict warnings when overlapping planned batches. |

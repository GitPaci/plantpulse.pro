...
import { firstNonShutdownStart, reconstructCanonicalChainPaths, validateSchedule } from './schedule-validation';
...
describe('validateSchedule', () => {
  ...
  it('shifts stage start to first valid time after shutdown overlap', () => {
    const start = d('2026-08-01T00:00:00Z');
    const adjusted = firstNonShutdownStart(start, 4 * 3600 * 1000, [
      { id: 'sd', name: 'Shutdown', startDate: d('2026-08-01T02:00:00Z'), endDate: d('2026-08-01T06:00:00Z') },
    ]);
    expect(adjusted.toISOString()).toBe('2026-08-01T06:00:00.000Z');
  });

  it('flags disconnected chain when post-shutdown continuity is not restored', () => {
    const linkChain: BatchChain = { ...chain, id: 'c6', linkToNext: true };
    const defaults6: StageDefault[] = [
      { stageType: 'seed', defaultDurationHours: 4, machineGroup: 'inoculum' },
      { stageType: 'production', defaultDurationHours: 8, machineGroup: 'fermenter' },
    ];
    const stages: Stage[] = [
      { id: 'c6s1', batchChainId: 'c6', machineId: 'm-ino', stageType: 'seed', startDatetime: d('2026-08-01T06:00:00Z'), endDatetime: d('2026-08-01T10:00:00Z'), state: 'planned' },
      { id: 'c6s2', batchChainId: 'c6', machineId: 'm-fer', stageType: 'production', startDatetime: d('2026-08-01T12:00:00Z'), endDatetime: d('2026-08-01T20:00:00Z'), state: 'planned' },
    ];
    const result = validateSchedule({ stages, batchChains: [linkChain], machines, stageDefaultsByProductLine: { pl1: defaults6 } });
    expect(result.valid).toBe(false);
    expect(result.issues.some((i) => i.code === 'CHAIN_LINK_CONTINUITY')).toBe(true);
  });
});
